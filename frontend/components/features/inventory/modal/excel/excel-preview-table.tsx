...
export default function ExcelPreviewTable({
  ...
  const getVisibleColumns = () => {
    ...
    if (importType === "medicine_batch" || importType === "medicine_only") {
      baseColumns.push("generic_name", "unit", "supplier", "price");
    }

    if (importType === "medicine_batch" || importType === "batch_only") {
          ...
          <div className="flex items-start gap-2">
            ...
            <div>
              ...
              <p className="mt-1 text-sm text-red-700">
                Rows with errors cannot be imported. Please fix the issues
                before proceeding.
              </p>
            </div>
          </div>
            ...
            <thead>
              <tr className="border-b border-slate-200 bg-slate-50">
                ...
                {visibleColumns.map((column) => (
                  <th
                    key={column}
                    className="p-3 text-left font-medium whitespace-nowrap text-slate-700"
                  >
                    {getColumnLabel(column)}
                  </th>
                ))}
                <th className="p-3 text-left font-medium whitespace-nowrap text-slate-700">
                  Status
                </th>
              </tr>
            </thead>
                ...
                >
                  ...
                  {visibleColumns.map((column) => (
                    <td
                      ...
                      className={`p-3 whitespace-nowrap text-slate-700 ${
