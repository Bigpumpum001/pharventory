...
export default function DeleteMedicineModal({
  ...
  const { deleteMedicineOpen, deleteMedicineId, closeDeleteMedicine } =
    useInventoryUI();
  ...
  const confirm = async () => {
    ...
    setIsLoading(true);
    try {
      const res = await fetch(`/api/medicines/${deleteMedicineId}`, {
        method: "DELETE",
      });
    ...
    <Dialog open={deleteMedicineOpen} onOpenChange={closeDeleteMedicine}>
      <DialogContent className="max-w-md border-slate-200 bg-white">
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <DialogDescription className="mt-1 text-slate-600">
                This action will soft-delete the medicine. You can restore it
                later.
              </DialogDescription>
            </div>
        ...
        <div className="mt-6 flex justify-end gap-3">
          <Button variant="outline" onClick={closeDeleteMedicine}>
            Cancel
          </Button>
          <Button
