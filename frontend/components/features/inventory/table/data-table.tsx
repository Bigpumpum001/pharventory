...
export function DataTable<Medicines>({
  ...
  return (
    <div className="w-full overflow-x-auto rounded-b-sm border-t-slate-800">
              ...
              <TableRow key={headerGroup.id} className="border-b-slate-400/90">
                {headerGroup.headers.map((header) => {
                  return (
                    <TableHead
                      key={header.id}
                      className={`p-2 px-3 text-slate-900 sm:p-2 sm:px-5 ${header.column.columnDef.meta?.headerClassName || ""} `}
                    >
              ...
              pageRows.map((row) => (
                ...
                >
                  {row.getVisibleCells().map((cell) => (
                    <TableCell
                      key={cell.id}
                      className={`p-2 px-3 text-sm text-slate-900 sm:p-5 sm:text-base ${cell.column.columnDef.meta?.cellClassName || ""} `}
                    >
                      {flexRender(
      ...
      {/* Pagination controls */}
      <div className="flex items-center justify-center gap-2 bg-slate-900/20 px-3 py-4">
        <Button
