...
describe('UnitsService', () => {
  ...
  describe('findOne', () => {
    ...
    it('should throw NotFoundException when unit not found', async () => {
      ...
      const promise = service.findOne(99);

      await expect(promise).rejects.toThrow(NotFoundException);
      await expect(promise).rejects.toThrow('Unit with ID 99 not found');
    });
  });
  ...
  describe('update', () => {
    ...
    it('should throw BadRequestException when new name belongs to another unit', async () => {
      ...
      (unitRepo.findOne as jest.Mock).mockImplementation(
        (options: { where?: Partial<Unit> }) => {
          ...
          if (options.where?.name === 'ml') {
            return Promise.resolve(otherUnit);
          }
      ...
      await expect(promise).rejects.toThrow(BadRequestException);
      await expect(promise).rejects.toThrow(
        'Unit with name "ml" already exists in the system',
      );
    });
    ...
    it('should throw NotFoundException when unit not found', async () => {
      ...
      const promise = service.update(99, { name: 'ml' });
  ...
  describe('remove', () => {
    ...
    it('should throw NotFoundException when unit not found', async () => {
      ...
      const promise = service.remove(99);
